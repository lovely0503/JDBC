...
public class AnimalView {
	...
	public void mainMenu() {
		//실습
		//테이블 두개 설계하기 -> ERD Cloud로 설계 -> 설명하기
		//테이블을 만든이유 -> 컬럼 구성 이유 -> 제약조건 이유->OK
		//내가 만든 테이블로 1~8번 구현하기(이 버전으로!)
		while(true) {
			...
			case "6" : selectKeeperByCondtion();break;
	...
	private void selectKeeperByCondtion() {
		System.out.println("사육사 검색 서비스 입니다");
		...
		new KeeperController().selectKeeperByCondtion(arguments); 
	...
	private void updateAnimal() {
		...
		System.out.println("변경하실 구역번호를 입력해주세요 >");
		String zoneId = sc.nextLine();
		System.out.println("변경하실 사육사번호를 입력해주세요 > ");
		String keeperId = sc.nextLine();
		...
		if(result > 0) {
			System.out.println("정보수정에 성공했습니다");
		}else {
